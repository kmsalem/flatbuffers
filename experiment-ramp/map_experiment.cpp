#include <iostream>
#include <utility>
#include "map_experiment_generated.h"  // includes "flatbuffer_ramp.h"
...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        ...
        std::cerr << "./map_experiment path_to_config id container_size(in bytes) bucket_size_in_milli_seconds" << std::endl;
        return 1;
    }
    ...
    if (id == 0) {
        ...
        while (key < num_entries) {
            m->testMap2[(int32_t)key] = m->CreaterString(str);
            key++;
        }
        ...
        while (access < 4000000) {
            val = m->testMap2[distr(generator)];
            access++;
        }
        ...
        while(true) {
            ...
            val = m->testMap2[distr(generator)];   // where read data happens?
            auto e1 = std::chrono::high_resolution_clock::now();
            // if (bin == 1) printf("---------------------------- val read is %s ------------------------\n", val.c_str());
