...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        ...
        std::cerr << "./vector_experiment3 path_to_config id container_size(in bytes) bucket_size_in_milli_seconds" << std::endl;
        return 1;
    }
    ...
    if (id == 0) {
        ...
        while (key < num_entries) {
            m->testVector3.push_back(m->CreateObj<ChildT>());
            m->testVector3[key]->pos = m->CreateObj<Vec3T>();
            m->testVector3[key]->pos->x = key - 1;
            m->testVector3[key]->pos->y = key;
            m->testVector3[key]->pos->z = key + 1;
            key++;
        }
        ...
        while (access < 4000000) {
            val = m->testVector3[distr(generator)]->pos->x;
            access++;
        }
        ...
        while(true) {
            ...
            val = m->testVector3[distr(generator)]->pos->z;   // where read data happens
